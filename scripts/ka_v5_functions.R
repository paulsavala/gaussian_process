...
#' @param df.points A spatial dataframe containing the column `pm2_5`, which is normalized between 0 and 1.
...
generate_knot_metrics = function(df.points, n.neighbors) {
  metric.df = data.frame(df.points) %>% subset(select=c(x, y, pm2_5))
  for (i in 1:nrow(df.points)) {
    ...
    metric.df[i, "avg_pm2_5"] = df.points.subset$pm2_5 %>% mean
    metric.df[i, "entropy"] = df.points.subset$pm2_5 %>% ebrahimi_entropy
  }
  metric.df
}
...
ss_new = function(df.km, radius.mult, max.knots, cols.to.sort) {
  ...
  for (i.knot in 1:max.knots) {
    ...
    # update df.km with new pm2_5
    is.neighbor = euclid_dist_2d(df.km$x, df.km$y, df.km[1, "x"], df.km[1, "y"]) <= df.km[1, "radius"] * radius.mult
    df.km$pm2_5[is.neighbor] = df.km$pm2_5[is.neighbor] - df.km[1, "avg_pm2_5"] #.5
...
eval_knots_mse = function(df.points, df.knots, gam.k=3) {
  gam.eval = gam(pm2_5 ~ te(x, y, k=gam.k, bs="gp"),data=df.knots,method="REML", family=gaussian)
  df.points[, "pkn"] = predict(gam.eval, newdata=df.points)
  (df.points$pm2_5 - df.points[, "pkn"])^2 %>% mean  # mse
}
